...
class MailClient():
    ...
    def loadmail(self):
        ...
        for count, mail in enumerate(emails):
            ...
            self.__setattr__(f'mail_{mail.replace(".", "_")}', lambda: open_eml_file())
            self.__setattr__(mail.replace(".", "_"), tkinter.Button(self.main_window, text=mail, command=self.__getattribute__(f'mail_{mail.replace(".", "_")}')))
            self.__getattribute__(mail.replace(".", "_")).grid(row=count, column=0)
